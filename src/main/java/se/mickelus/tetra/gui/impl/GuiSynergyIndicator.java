...
public class GuiSynergyIndicator extends GuiElement {
    ...
    public GuiSynergyIndicator(int x, int y) {
        ...
        indicator = new GuiTexture(0, 0, width, height, 176, 0, texture);
        addChild(indicator);
    }
    ...
    public void update(ItemStack itemStack, ItemModule module) {
        ...
        tooltip.add(TextFormatting.GRAY + I18n.format("item.modular.synergy_indicator.header"));
        ...
        if (tooltip.size() <= 1) {
            tooltip = Collections.singletonList(TextFormatting.GRAY + I18n.format("item.modular.synergy_indicator.empty"));
