...
public class TetraAttributes {
    public static final DeferredRegister<Attribute> registry = DeferredRegister.create(ForgeRegistries.ATTRIBUTES, TetraMod.MOD_ID);
    ...
    public static void onEntityAttributeModification(final EntityAttributeModificationEvent event) {
        registry.getEntries().forEach(attribute -> event.add(EntityType.PLAYER, attribute.get(), attribute.get().getDefaultValue()));
    }
}
